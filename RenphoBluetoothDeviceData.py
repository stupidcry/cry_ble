...
class RenphoBluetoothDeviceData(BluetoothData):
    ...
    def _start_update(self, service_info: BluetoothServiceInfo) -> None:
        _LOGGER.w("===*** _start_update")

    def poll_needed(
        self, service_info: BluetoothServiceInfo, last_poll: float | None
    ) -> bool:
        _LOGGER.w("=== poll_needed")
        return True

    @retry_bluetooth_connection_error()
    async def _get_payload(self, client: BleakClientWithServiceCache) -> None:
        _LOGGER.w("=== _get_payload")

    async def async_poll(self, ble_device: BLEDevice) -> SensorUpdate:
        _LOGGER.w("=== Polling device: %s", ble_device.address)
        ...
        try:
            ...
            _LOGGER.w(f"Reading gatt characters failed with err: {err}")
        finally:
